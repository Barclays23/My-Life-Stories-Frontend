...
const Book = () => {
  ...
  const [pageLoading, setPageLoading] = useState(true); // For page navigation
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (bookDoc.exists()) setBook({ id: bookDoc.id, ...bookDoc.data() });

        const chaptersSnapshot = await getDocs(collection(db, 'chapters'));
        const chaptersList = chaptersSnapshot.docs
          .map(doc => ({ id: doc.id, ...doc.data() }))
          .filter(chapter => chapter.bookRef === bookId)
          .sort((a, b) => a.chapterNumber - b.chapterNumber);
        setChapters(chaptersList);
        ...
        const reviewsList = reviewsSnapshot.docs
          .map(doc => ({ id: doc.id, ...doc.data() }))
          .filter(review => review.bookRef === bookId)
          .slice(0, 5);
        setReviews(reviewsList);
      } catch (error) {
  ...
  const handleAddChapter = async () => {
    ...
    setFormLoading(true);
    try {
      const response = await api.createChapter(token, { bookId, chapterTitle: newChapterTitle });
      setChapters([...chapters, {
        id: response.id,
        bookRef: bookId,
        chapterTitle: newChapterTitle,
  ...
  const handleDeleteChapter = async (chapterId, chapterNumber) => {
    setFormLoading(true);
    try {
      await api.deleteChapter(token, { chapterId, bookId, chapterNumber });
  ...
  const handlePublish = async () => {
    ...
    setFormLoading(true);
    try {
      await api.publishBook(token, { bookId, publicationDate });
      const pubDate = new Date(publicationDate);
      const today = new Date();
      let releaseStatus = 'Draft';
      const daysDiff = (today - pubDate) / (1000 * 60 * 60 * 24);
      if (pubDate > today) releaseStatus = 'Coming Soon';
      else if (daysDiff <= 14) releaseStatus = 'New Release';
  ...
  const handlePayment = async () => {
    setFormLoading(true);
    try {
      const order = await api.createOrder(token, { amount: book.price * 100, currency: 'INR' });
      const options = {
  ...
  if (!book) return <div>Error loading book data.</div>;

  return (
    <div className="container mx-auto py-8">
      <div className="bg-gray-100 dark:bg-gray-700 p-6 rounded-lg mb-8 flex flex-col md:flex-row items-center">
        <img
          src={book.coverImage || '/assets/images/default-book-image.jpg'}
          alt={book.title}
          className="w-48 h-64 object-cover rounded-md mr-6 mb-4 md:mb-0"
        />
        <div>
          ...
          <p>Access: {book.accessType === 'Free' ? 'Free' : `₹${book.price}`}</p>
          <p>Rating: {book.ratingAverage.toFixed(1)} ({book.ratingCount} reviews)</p>
          {book.accessType === 'Paid' && user && !isAdmin && (
            <button
              onClick={handlePayment}
              className="btn bg-blue-600 text-white px-4 py-2 rounded mt-4"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {chapters.map(chapter => (
          <div key={chapter.id} className="relative">
            <StoryCard
              story={{ id: chapter.id, title: chapter.chapterTitle, content: `Chapter ${chapter.chapterNumber}` }}
